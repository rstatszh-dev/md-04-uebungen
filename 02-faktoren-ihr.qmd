...
1.  Erkunde die Daten mittels der `count` Funktion um die Reihenfolge der Variablen `alter` und `geschlecht` zu ermitteln. Sind diese ordinal skaliert (haben sie eine natürliche Ordnung?). Falls ja, muss diese mittels einer Faktorisierung festgelegt werden?
...
1.  Definiere die Reihenfolge der Faktoren für die Variable `antwort`. Speichere in einem Vektor mittels der `c()` und gebe dem Objekt den Namen `antwort_levels`
...
1.  Starte mit dem Daten-Objekt `befragung_fct` und erstelle eine Häufigkeitstabelle für die Variable `antwort`, gruppiert nach `alter`. Nutze die Funktionen `group_by()` und `summarise()`. Speichere das Ergebnis in einem neuen Objekt `befragung_sum_alter`.
...
1.  Starte mit dem Daten-Objekt `befragung_fct` und erstelle eine Häufigkeitstabelle für die Variable `antwort`, gruppiert nach `geschlecht`. Nutze die Funktionen `group_by()` und `summarise()`. Speichere das Ergebnis in einem neuen Objekt `befragung_sum_geschlecht`.
